...
test.describe("role-based access", () => {
  test("viewer is read-only; supervisor can mark attendance but not manage settings", async ({
    ...
    await goto(ownerPage, "/settings/members");
    ...
    await goto(memberPage, "/dashboard");
    ...
    await goto(memberPage, "/settings");
    await expect(memberPage).toHaveURL(/\/dashboard/, { timeout: 15_000 });
    await goto(memberPage, "/attendance");
    await expect(memberPage).toHaveURL(/\/dashboard/, { timeout: 15_000 });
    ...
    await goto(ownerPage, "/attendance");
    ...
    await expect(changeRoleForm.locator('select[name="role"]')).toHaveValue("supervisor", { timeout: 20_000 });
