...
const baseReport: ReportData = {
  ...
  rows: [
    ...
    { worker_name: "Ruwan Fernando", worker_code: "W003", status: "unknown", in_time: null, out_time: null, note: null },
  ],
  updated_at: "14:05",
...
describe("buildReportText", () => {
  ...
  it("counts statuses correctly", () => {
    const text = buildReportText(baseReport);
    expect(text).toContain("Present: 1 | Absent: 1 | Not marked: 1");
  });
  ...
  it("lists workers in order with status and time", () => {
    ...
    expect(text).toContain("3. Ruwan Fernando (W003) — Not marked");
  });
  ...
  it("handles an empty worker list", () => {
    ...
    expect(text).toContain("Present: 0 | Absent: 0 | Not marked: 0");
  });
...
describe("countStatuses", () => {
  it("counts only known statuses", () => {
    ...
    expect(counts).toEqual({ present: 2, absent: 1, unknown: 1 });
  });

  it("returns zeros for an empty list", () => {
    expect(countStatuses([])).toEqual({ present: 0, absent: 0, unknown: 0 });
  });
});
